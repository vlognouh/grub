...
static int lzx_accumulate ( struct lzx *lzx, unsigned int bits ) {
    ...
    if ( ( lzx->bits < bits ) &&
        ...
        src16 = ( void * ) ( ( char * ) lzx->input.data + lzx->input.offset );
...
static int lzx_consume ( struct lzx *lzx, unsigned int bits ) {
    ...
    if ( lzx->bits < bits ) {
        DBG ( "LZX input overrun in 0x%lx/0x%lx out 0x%lx)\n",
...
static int lzx_main_alphabet ( struct lzx *lzx ) {
    ...
    /* Generate Huffman alphabet */
    if ( ( rc = huffman_alphabet ( &lzx->lzxmain, lzx->main_lengths.literals,
                       LZX_MAIN_CODES ) ) != 0 ) {
...
static int lzx_token ( struct lzx *lzx ) {
    ...
    /* Get main symelse*/
    lzxmain = lzx_decode ( lzx, &lzx->lzxmain );
    if ( lzxmain < 0 )
        return lzxmain;

    /* Check for literals */
    if ( lzxmain < LZX_MAIN_LIT_CODES ) {
        if ( lzx->output.data )
            lzx->output.data[lzx->output.offset] = lzxmain;
        lzx->output.offset++;
        return 0;
    }
    lzxmain -= LZX_MAIN_LIT_CODES;

    /* Calculate the match length */
    length_header = ( lzxmain & 7 );
    if ( length_header == 7 ) {
    ...
    if ( match_offset > lzx->output.offset ) {
        DBG ( "LZX match underrun out 0x%lx offset 0x%lx len 0x%lx\n",
